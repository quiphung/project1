...
/**
        refix: File hình ảnh sẽ tự động xóa sau 24h
...
class Antispam {
	...
	*/
	...
	var $char_set 		= "qwertyuioplkjhgfdsazxcvbnm2345689";	//CODE_ALLOWED_CHARS
	...
	function get_antispam_image( $override = array() ) {
		...
		$img_markup = '<img src="' . $this->img_url . $img_name . '" width=" ' . $this->img_width
					. '" height="' . $this->img_height . '" style="border:0;" alt=" " id="imgcaptcha" />';
	...
	function collect_files($dir, $ext) {
        ...
        function do_cleanup(){
            $ext='jpg';
            if (false !== ($dir = opendir($this->img_path))) {
                $files = array();
            ...
            //get info file and delete old file 
            
            $CI =& get_instance();
            $CI->load->helper('file');
            $CI->load->helper('date');
            $image_path='./captcha/image/';
            $aryFilenames = get_filenames($image_path,false,false);
            $returned_values = array('date');
            foreach ($aryFilenames as $value) {
                $path=$image_path.$value;
                $file_info=get_file_info($path, $returned_values);
                if ($this->check_time($file_info['date'])):
                    unlink($path);
                endif;
