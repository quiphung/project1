	...
	public function insert(){
		if($this->input->post('tieude')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$query = $this->db->get_where('danhmuc',array('tieude'=>$tieude));
				if($query->result()){
					$upload = 0;
				...
				if($upload==1){
					...
					$dataInsert = array(
						...
						'alias'			=>	$alias = $this->check->checkurldanhmuc($this->check->url_slug($tieude)),
						'thutu'			=> 	set_value('thutu'),
						'anhien'		=>	$anhien,
						'title'			=> 	$tieude,
						'description'	=> 	strip_tags($this->input->post('mota')),
					);
					if(!$this->db->insert('danhmuc',$dataInsert)){
						...
						if($this->input->post('goedit')!==null)
							redirect('admin/danhmuc/update/'.$this->db->insert_id());
					}
				}
			}
		}
		...
		$data['csrf'] = $csrf;
		$data['view'] = 'backend/modules/danhmuc/insert';
		$this->load->view('backend/layout/home',$data);
	}
